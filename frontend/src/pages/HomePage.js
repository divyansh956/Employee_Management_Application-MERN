...
const HomePage = () => {
  ...
  const deleteUser = async (id) => {
    try {
      const deleteUser = await fetch(`${process.env.REACT_APP_BASE_URL}/deleteUser/${id}`, {
        method: "DELETE",
      });
                  ...
                  <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                    {empData?.data.map((person) => (
                      <tr key={person._id}>
                        ...
                        <td className="px-4 py-4 whitespace-nowrap text-center text-sm text-gray-500 dark:text-gray-300">
                          <button className="text-lg"><MdDelete onClick={() => deleteUser(person._id)} /></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
